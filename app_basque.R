...
legend("topleft",legend=c("Basque Country","Controls"), seg.len=2, col=c("black","darkgray"),fill=NA,border=NA, lty=c(1,1),lwd=c(5,0.5), merge=T,bty="n")
dev.off()
...
for (k in 1:length(K.vec)){
  ...
  r.sc.sim          <- scinference(Y1,Y0,T1=T1,T0=T0,inference_method="ttest",estimation_method="sc",K=K.vec[k])
